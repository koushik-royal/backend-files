...
from fastapi import BackgroundTasks, UploadFile, File as FastAPIFile
...
def get_game_sessions_for_patient(
    ...
    """
    Returns game sessions for a given patient.
    Constraint: Doctors MUST provide a date_filter to see history.
    """
    ...
    if current_user.role == models.UserRole.DOCTOR:
        ...
        # --- NEW REQUIREMENT ---
        # If doctor hasn't picked a date, return empty list (then only it have to show)
    ...
    sessions = crud.get_game_sessions_by_user(db=db, user_id=patient_id, date_filter=date_filter)
    return sessions
...
async def upload_profile_image(
    ...
    """Upload and set the profile image for the authenticated user."""
    ...
    file_path = os.path.join("static/profile_images", file_name)
    
    with open(file_path, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)
    
    # Update user's profile_image_url
    url = f"/static/profile_images/{file_name}"
    user_update = schemas.UserUpdate(profile_image_url=url)
...
def delete_patient_account(
    ...
    """Allow doctors to remove patients from their list (disconnects them)."""
    if current_user.role != models.UserRole.DOCTOR:
         return JSONResponse(status_code=403, content={"detail": "Only doctors can remove patients"})
    
    success = crud.disconnect_patient(db=db, patient_id=user_id, doctor_id=current_user.id)
    if not success:
         return JSONResponse(status_code=404, content={"detail": "Patient not found or not linked to you"})
         
    return JSONResponse(status_code=200, content={"message": "Patient disconnected successfully"})
