...
# Function moved to bottom to avoid duplication and consolidated logic.
...
def get_conversation_partners_for_user(
    ...
    # Filter: Only show currently linked users due to strict Doctor-Patient relationship
    visible_partners = []
    
    if user:
        if user.role == models.UserRole.DOCTOR:
            # Doctor sees only currently linked patients
            for p in partners:
                if p.linked_doctor_id == user.id:
                    visible_partners.append(p)
                else:
                    print(f"DEBUG: Filtering out {p.id} (Linked Dr: {p.linked_doctor_id}) for Doctor {user.id}")
        elif user.role == models.UserRole.PATIENT:
            # Patient sees only their currently linked doctor
            for p in partners:
                if p.id == user.linked_doctor_id:
                    visible_partners.append(p)
        else:
            # Admins or others see everyone
...
def disconnect_patient(db: Session, patient_id: int, doctor_id: int):
    ...
    """
    print(f"DEBUG: Attempting to disconnect Patient {patient_id} from Doctor {doctor_id}")
    patient = get_user_by_id(db, user_id=patient_id)
    if not patient:
        print("DEBUG: Patient not found")
        return False
    
    print(f"DEBUG: Patient Found. Linked Doctor ID: {patient.linked_doctor_id}")
    if patient.linked_doctor_id != doctor_id:
        print(f"DEBUG: Mismatch! Patient linked to {patient.linked_doctor_id}, not {doctor_id}")
        return False
        
    patient.linked_doctor_id = None
    db.commit()
    print("DEBUG: Disconnection Successful. Commited to DB.")
    return True

# --- Doctor Game History Helpers ---
def get_game_sessions_by_user(db: Session, user_id: int, date_filter: date = None):
    """
    Returns all game sessions for a given patient/user.
    Optionally filtered by date (ignoring time).
    """
    query = db.query(models.GameSession).filter(models.GameSession.patient_id == user_id)
    
    if date_filter:
        # Robust date extraction for SQLite/Postgres/MySQL
        target_date_str = date_filter.isoformat()
        query = query.filter(func.date(models.GameSession.date_played) == target_date_str)
        
    sessions = query.order_by(models.GameSession.date_played.desc()).all()
    return sessions
